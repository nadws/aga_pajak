        ...
        <table class="table table-bordered" id="tblAldi2">
            <thead>
                <tr>
                    ...
                    <th class="dhead text-center" colspan="6">Sortir</th>
            ...
            <tbody>
                @foreach ($bk as $no => $g)
                    ...
                    <tr>
                        ...
                        <td class="text-center">{{ $g->pcs_awal_sortir ?? 0 }}</td>
                        <td class="text-center">{{ $g->gr_awal_sortir ?? 0 + $gr_eo_awal }}</td>
                        <td class="text-center">{{ $g->pcs_akhir_sortir ?? 0 }}</td>
                        <td class="text-center">{{ $g->gr_akhir_sortir ?? 0 }}</td>
                        @php
                            $pcs_awal_bk = $g->pcs_awal ?? 0;
                            $gr_awal_bk = $g->gr_awal ?? 0;

                            $pcs_awal_cbt = $g->pcs_awal_sortir ?? 0;
                            $gr_awal_cbt = $g->gr_awal_sortir ?? 0;
                            $pcs_akhir_cbt = $g->pcs_akhir_sortir ?? 0;
                            $gr_akhir_cbt = $g->gr_akhir_sortir ?? 0;
